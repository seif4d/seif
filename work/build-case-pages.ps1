...
$projects = @(
  @{
    ...
    BuiltEn='The current public portfolio evidence includes the web experience, responsive presentation and visual direction. The exact commerce scope and measured business results still need confirmation before they are presented as facts.'; BuiltAr='الدليل العام المتاح حاليًا يشمل تجربة الويب والعرض المتجاوب والتوجيه البصري. ما زال نطاق التجارة والنتائج التجارية المقاسة بحاجة إلى تأكيد قبل عرضها كحقائق.';
  ...
  @{
    ...
    BuiltEn='The wider project context includes a modern frontend, a Laravel-based application layer and an administration workflow. The final public case study will be updated from the source project before launch.'; BuiltAr='يتضمن سياق المشروع الأوسع واجهة حديثة وطبقة تطبيق مبنية على Laravel ومسارًا للإدارة. ستُحدّث دراسة الحالة النهائية من المشروع الأصلي قبل الإطلاق.';
  ...
  @{
    ...
    BuiltEn='The public project demonstrates a responsive JavaScript reading interface and content interaction. AI, semantic-search and offline claims are intentionally excluded here until they are verified against the current implementation.'; BuiltAr='يعرض المشروع العام واجهة قراءة متجاوبة مبنية بجافاسكربت وتفاعلًا مع المحتوى. استُبعدت ادعاءات الذكاء الاصطناعي والبحث الدلالي والعمل دون اتصال حتى التحقق من التنفيذ الحالي.';
  ...
  @{
    ...
    BuiltEn='DRAGON is currently presented as a venture concept. The portfolio does not claim a launched platform, active community size or measured learning outcome.'; BuiltAr='يُعرض DRAGON حاليًا باعتباره تصورًا لمشروع. لا يدّعي البورتفوليو وجود منصة مطلقة أو حجم مجتمع نشط أو نتائج تعلم مقاسة.';
  ...
  @{
    ...
    BuiltEn='SCOPE is shown as Seif''s venture-building direction. Claims about portfolio companies, sprint speed or market validation are withheld until evidence is ready to publish.'; BuiltAr='يُعرض SCOPE بوصفه اتجاه سيف في بناء المشاريع. تُحجب ادعاءات الشركات التابعة أو سرعة التنفيذ أو تحقق السوق حتى تصبح الأدلة جاهزة للنشر.';
  ...
  @{
    ...
    BuiltEn='The public project demonstrates the responsive web and brand presentation. Conversion improvement is treated as a design intention, not a measured result, until analytics evidence is available.'; BuiltAr='يعرض المشروع العام تجربة الويب المتجاوبة والتقديم البصري للعلامة. يُعامل تحسين التحويل كهدف تصميمي لا كنتيجة مقاسة حتى تتوفر بيانات تحليل موثقة.';
...
<body data-title-en="$($p.Name) — SEIF4D project" data-title-ar="$($p.Name) — مشروع SEIF4D" data-description-en="$($p.LedeEn)" data-description-ar="$($p.LedeAr)">
  ...
  <main>
    <section class="case-hero shell">
      <div class="en-copy"><p class="eyebrow">$($p.TypeEn)</p><h1>$($p.Name)</h1><p class="lede">$($p.LedeEn)</p></div>
      <div class="ar-copy"><p class="eyebrow">$($p.TypeAr)</p><h1>$($p.Name)</h1><p class="lede">$($p.LedeAr)</p></div>
      <img class="cover" src="../assets/$($p.Image)" width="1448" height="1086" alt="$($p.Name) editorial project cover">
      <dl class="proof en-copy"><div><dt>Role</dt><dd>$($p.RoleEn)</dd></div><div><dt>Status</dt><dd>$($p.StatusEn)</dd></div><div><dt>Year</dt><dd>$($p.Year)</dd></div><div><dt>Evidence</dt><dd>Portfolio record</dd></div></dl>
      <dl class="proof ar-copy"><div><dt>الدور</dt><dd>$($p.RoleAr)</dd></div><div><dt>الحالة</dt><dd>$($p.StatusAr)</dd></div><div><dt>السنة</dt><dd>$($p.Year)</dd></div><div><dt>الدليل</dt><dd>سجل البورتفوليو</dd></div></dl>
    </section>
    <section class="case-content shell">
      <div class="copy en-copy"><h2>From context<br>to a clearer product.</h2><h3>The challenge</h3><p>$($p.ProblemEn)</p><h3>What is evidenced</h3><p>$($p.BuiltEn)</p><h3>One product decision</h3><p>$($p.DecisionEn)</p>$liveEn</div>
      <div class="copy ar-copy"><h2>من السياق<br>إلى منتج أوضح.</h2><h3>التحدي</h3><p>$($p.ProblemAr)</p><h3>ما تؤكده الأدلة</h3><p>$($p.BuiltAr)</p><h3>قرار في المنتج</h3><p>$($p.DecisionAr)</p>$liveAr</div>
      <aside class="note"><span class="status"><span class="en-copy">$($p.StatusEn)</span><span class="ar-copy">$($p.StatusAr)</span></span><div class="en-copy"><b>Evidence before claims</b><p>This first case-study version separates confirmed public material from outcomes that still need source data or permission.</p></div><div class="ar-copy"><b>الدليل قبل الادعاء</b><p>تفصل النسخة الأولى من دراسة الحالة بين المواد العامة المؤكدة والنتائج التي ما زالت تحتاج إلى مصدر أو تصريح.</p></div></aside>
    </section>
